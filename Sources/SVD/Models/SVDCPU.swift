...
public struct SVDCPU {
  ...
  ///
  /// The version format is `rNpM` where `(N,M = [0 - 99])`.
  ...
  /// point unit (FPU).
  ///
  /// Cortex-M4, Cortex-M7, Cortex-M33 and Cortex-M35P are the only available
  /// Cortex-M processor with an optional FPU. This tag is either set to true or
  /// false, 1 or 0.
  public var fpuPresent: Bool
  /// Indicate whether the processor is equipped with a double precision
  /// floating point unit.
  ///
  /// This element is valid only when `<fpuPresent>` is set to true. Currently,
  /// only Cortex-M7 processors can have a double precision floating point unit.
  public var fpuDP: Bool?
  /// Indicates whether the processor implements the optional SIMD DSP
  /// extensions (DSP).
  ///
  /// Cortex-M33 and Cortex-M35P are the only available Cortex-M processor with
  /// an optional DSP extension. For ARMv7M SIMD DSP extensions are a mandatory
  /// part of Cortex-M4 and Cortex-M7. This tag is either set to true or false,
  /// 1 or 0.. This element is mandatory for Cortex-M33, Cortex-M35P and future
  /// processors with optional SIMD DSP instruction set.
  ...
  /// in Cortex-M0+ based devices.
  ///
  /// This tag is either set to true or false, 1 or 0. If not specified, then
  /// VTOR is assumed to be present.
