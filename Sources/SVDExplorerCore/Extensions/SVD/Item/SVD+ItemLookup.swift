...
extension SVDDocument {
  ...
  func item(
    ...
    guard !keyPath.isEmpty else {
      throw SVDLookupItemNotFoundError(
        parentName: "<root>", childName: "<none>")
    }
    let key = keyPath.removeFirst()
    guard key.kind == .device, self.device.name == key.name else {
      throw SVDLookupItemNotFoundError(
        parentName: "<root>", childName: key.name)
    }
...
extension SVDDevice {
  func item(
    ...
    let key = keyPath.removeFirst()
    guard key.kind == .peripheral,
      let item = self.peripherals.peripheral.first(where: {
        $0.name == key.name
      })
    else {
      throw SVDLookupItemNotFoundError(
        parentName: self.name, childName: key.name)
    }
...
extension SVDPeripheral {
  func item(
    ...
    let key = keyPath.removeFirst()
    if key.kind == .cluster,
      let item = self.registers?.cluster.first(where: { $0.name == key.name })
    {
      return try item.item(at: keyPath)
    } else if key.kind == .register,
      let item = self.registers?.register.first(where: { $0.name == key.name })
    {
      return try item.item(at: keyPath)
    } else {
      throw SVDLookupItemNotFoundError(
...
extension SVDCluster {
  func item(
    ...
    let key = keyPath.removeFirst()
    if key.kind == .cluster,
      let item = self.cluster?.first(where: { $0.name == key.name })
    {
      return try item.item(at: keyPath)
    } else if key.kind == .register,
      let item = self.register?.first(where: { $0.name == key.name })
    {
      return try item.item(at: keyPath)
    } else {
      throw SVDLookupItemNotFoundError(
...
extension SVDRegister {
  func item(
    ...
    let key = keyPath.removeFirst()
    guard key.kind == .field,
      let item = self.fields?.field.first(where: { $0.name == key.name })
    else {
      throw SVDLookupItemNotFoundError(
        parentName: self.name, childName: key.name)
    }
