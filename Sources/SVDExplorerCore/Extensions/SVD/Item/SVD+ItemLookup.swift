...
extension SVDDevice {
  func item(
    ...
    let key = keyPath.removeFirst()
    guard key.kind == .peripheral, let item = self.peripherals.peripheral.first(where: { $0.name == key.name }) else {
...
extension SVDRegister {
  func item(
    ...
    let key = keyPath.removeFirst()
    guard key.kind == .field, let item = self.fields?.field.first(where: { $0.name == key.name }) else {
      throw SVDLookupItemNotFoundError(parentName: self.name, childName: key.name)
    }
    return try item.item(at: keyPath)
  }
