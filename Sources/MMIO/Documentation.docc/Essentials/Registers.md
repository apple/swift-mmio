...
Swift MMIO uses macros to define type-safe register interfaces without manual bit manipulation.
...
The type `INTERRUPT_ENABLE` is automatically generated by the Swift MMIO macro system. It contains metadata about the bit field, including its position and width. By convention, these type names should match the field names in the hardware's datasheet; however Swift MMIO does not enforce this.
