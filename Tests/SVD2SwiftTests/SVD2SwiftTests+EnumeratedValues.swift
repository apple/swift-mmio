...
extension SVD2SwiftTests {
  ...
  @Test func enumeratedValues_reservedWords() throws {
    var device = Self.testEnumeratedValuesBasicDevice
    device.peripherals.peripheral[0].registers?.register[0].fields?.field[0]
      .enumeratedValues?.enumeratedValue = [
        .init(name: "Any", data: .value(0x0, mask: .max)),
        .init(name: "while", data: .value(0x1, mask: .max)),
        .init(name: "let", data: .value(0x2, mask: .max)),
        .init(name: "switch", data: .value(0b11100, mask: 0b11100)),
      ]
    var options = ExportOptions.testDefault
    ...
    assertSVD2SwiftOutput(
      ...
      expected: [
        ...
        extension ExamplePeripheral.ExampleRegister {
          public struct AValues: BitFieldProjectable, RawRepresentable {
            ...
            public static func ~= (pattern: Pattern, value: Self) -> Bool {
              (value.rawValue & pattern.mask) == pattern.rawValue
            }

            /// Any
            public static let `Any` = Self(rawValue: 0x0)

            /// while
            public static let `while` = Self(rawValue: 0x1)

            /// let
            public static let `let` = Self(rawValue: 0x2)

            /// switch
            public static let `switch` = Pattern(rawValue: 0x1c, mask: 0x1c)

            /// switch
            public static func `switch`(_ rawValue: UInt8 = `switch`.value) -> Self {
              let value = Self(rawValue: rawValue)
              precondition(`switch` ~= value, "Invalid bits set in rawValue")
              return value
            }
