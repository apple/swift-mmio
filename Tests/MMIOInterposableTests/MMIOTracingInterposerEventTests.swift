...
final class MMIOTracingInterposerEventTests: XCTestCase {
  func test_load() {
    XCTAssertEqual(
      ...
      MMIOTracingInterposerEvent(load: true, address: 0x10, size: 1, value: 1))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.load(of: UInt16(2), from: 0x10),
      MMIOTracingInterposerEvent(load: true, address: 0x10, size: 2, value: 2))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.load(of: UInt32(3), from: 0x10),
      MMIOTracingInterposerEvent(load: true, address: 0x10, size: 4, value: 3))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.load(of: UInt64(4), from: 0x10),
      MMIOTracingInterposerEvent(load: true, address: 0x10, size: 8, value: 4))
  }

  func test_store() {
    XCTAssertEqual(
      MMIOTracingInterposerEvent.store(of: UInt8(1), to: 0x10),
      MMIOTracingInterposerEvent(load: false, address: 0x10, size: 1, value: 1))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.store(of: UInt16(2), to: 0x10),
      MMIOTracingInterposerEvent(load: false, address: 0x10, size: 2, value: 2))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.store(of: UInt32(3), to: 0x10),
      MMIOTracingInterposerEvent(load: false, address: 0x10, size: 4, value: 3))
    XCTAssertEqual(
      MMIOTracingInterposerEvent.store(of: UInt64(4), to: 0x10),
      MMIOTracingInterposerEvent(load: false, address: 0x10, size: 8, value: 4))
  }
